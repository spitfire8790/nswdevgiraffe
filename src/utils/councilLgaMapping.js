...
// Create a reverse mapping from LGA names to council names
export const reverseLgaMapping = Object.entries(lgaMapping).reduce((acc, [council, lga]) => {
  acc[lga] = council;
  return acc;
}, {});

// Format LGA name with proper capitalization for display
...
// Get council name from LGA name
export const getCouncilFromLga = (lgaName) => {
  if (!lgaName) return null;
  // Try exact match first
  const council = reverseLgaMapping[lgaName];
  if (council) return council;
  
  // Try case-insensitive match
  const upperLgaName = lgaName.toUpperCase();
  const councils = Object.entries(lgaMapping);
  const match = councils.find(([_, lga]) => lga === upperLgaName);
  
  return match ? match[0] : null;
};

// Get all LGAs as an array of objects with display and value properties
export const getAllLgas = () => {
  return Object.entries(lgaMapping).map(([council, lga]) => ({
    display: formatLgaName(lga),
    value: lga,
    council
  }));
}; 